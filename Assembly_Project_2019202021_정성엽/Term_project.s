	AREA term_project, CODE, READONLY
		ENTRY
;R0=sampled data address
;R1=filter data address
;R2=count for filter
;R3~R8=calculate reg
;R9=count for sample data
;R10=tmp store address
;R12=multiplier result reg
;R11=sum of multiplier

Main
	ldr r0, =Sampled_Data ; save the data(480) using Sampled_Data label(32bit, double word).
	ldr r1, =Filter_Data
	MOV R2, #0; index
	LDR R10, TEMPADDR1;//address for multiply

MUL_FLT
	LDR r3,[r0]; sampled data
	LDR r4,[r1],#4;filter data
	...
	;exponent bit ADD
	MOV R5, R3, LSL #1;
	MOV R5, R5, LSR #24;get exponent
	
	MOV R6, R4, LSL  #1;
	MOV R6, R6, LSR #24;get exponent
	;add exponent 
	ADD R7, R5, R6;
	SUB R7, R7, #127;
	MOV R12, R7, LSL #23;
	;mantissa bit MUL
	MOV R5, R3, LSL #9;
	MOV R5, R5, LSR #9;get mantissa
	ADD R5, R5, #8388608;
	
	MOV R6, R4, LSL #9;
	MOV R6, R6, LSR #9;get mantissa
	ADD R6, R6, #8388608;
	
	UMULL R7,R8,R5, R6;
	MOV R7, R7, LSR #16;//get 16bit of lower
	MOV R8, R8, LSL #16;//get 16bit of higher
	ADD R7, R7, R8;
	
	CMP R7, #0x80000000;//if 1x.xxxxxx
	ADDGT R12, #8388608
	MOVGT R7, R7, LSL #1;
	MOVGT R7, R7, LSR #9;
	...
	STR R12,[R10],#4;store float multiply value to tempaddr1
	B MUL_FLT
...
SAVE_RE
	...
	CMP R9, #480;calculate before 480
	BEQ COMP_TMP
	B ADD_TMP
...
COMP_TMP
	...
	LDR R0, TEMPADDR2;
	LDR R1, =Compare_Data;
	LDR R13, Save_Result_Address;
	;compare ready
COMP
	ADD R12, R12, #1;index count
	CMP R12, #480;
	BEQ INDEX_TMP; goto index tmp
	
	LDR R2, [R0], #4;get sum value
	LDR R3, [R1];get compare value
	;;sign bit
...
YES;store value if lower than -4.7
	STR R2, [R13], #4;
	B COMP
	
INDEX_TMP;index store ready
	MOV R12, #0; index count
	LDR R0, TEMPADDR2
	LDR R1, =Compare_Data
...
YES_IN
	SUB R9, R12, #1;sub R12 for real index
	STRB R12, [R13], #2;store R12 to R13
	B INDEX
...
Filter_Data
	DCD 0x3FF99999, 0x3Fdc28F5, 0xBEDCAC08, 0xBFA39581, 0xBF4D5CFA, 0xBE589374
